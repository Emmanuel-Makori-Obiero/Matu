...
//   - how long the bus (live GPS) takes to reach the stage — Mapbox Directions
//     API, "driving-traffic" profile, so this ETA accounts for live traffic
//     conditions rather than a static free-flow estimate.
//   - how long the passenger takes to walk from where they are now to that
//     stage — OSRM's free public routing API (router.project-osrm.org), no
//     key needed. Traffic doesn't affect walking time, so no need to spend
//     Mapbox quota on this leg.
...
// Recomputing on every 5s GPS tick would burn through the Mapbox free tier
// (100k req/month) for no real benefit — ETAs don't meaningfully change that
// often. Refresh the bus leg on this cadence instead; walking time is
// recomputed only when the passenger's location or the target stage changes
// (both are far less volatile).
const BUS_ETA_REFRESH_MS = 20_000;

const MAPBOX_TOKEN = import.meta.env.VITE_MAPBOX_TOKEN as string | undefined;

async function osrmWalkingDurationSeconds(
  origin: LatLng,
  destination: LatLng,
): Promise<number | null> {
  try {
    const url = `https://router.project-osrm.org/route/v1/walking/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=false`;
...
// Traffic-aware driving ETA for the bus leg, via Mapbox's driving-traffic
// profile. Falls back to null (caller shows "couldn't calculate") if the
// token is missing or the request fails — it does NOT silently fall back to
// a non-traffic estimate, since a misleadingly-precise-looking ETA that
// ignores traffic is worse than an explicit error.
async function mapboxTrafficDurationSeconds(
  origin: LatLng,
  destination: LatLng,
): Promise<number | null> {
  ...
  try {
    const url =
      ...
      `?overview=false&annotations=duration&access_token=${MAPBOX_TOKEN}`;
    const res = await fetch(url);
    if (!res.ok) return null;
...
export function LeaveNowBanner({ busPos, stage }: { busPos: LatLng | null; stage: Stage | null }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const seconds = await osrmWalkingDurationSeconds(passengerPos, {
        lat: stage.lat,
        lng: stage.lng,
      });
      if (cancelled) return;
  ...
  // Bus leg: live bus position -> stage, traffic-adjusted. Refreshed on an
  // interval rather than every GPS update, to keep Mapbox usage reasonable.
  useEffect(() => {
    if (!stage) return;
    ...
    async function computeBusEta() {
      ...
      if (!pos) return;
      const seconds = await mapboxTrafficDurationSeconds(pos, {
        lat: stage!.lat,
        lng: stage!.lng,
      });
      if (seconds == null) {
    ...
    >
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
        <span className="inline-flex items-center gap-1">
          <Navigation2 className="size-3" /> Bus arrives in {busMinutes} min (traffic-adjusted)
        </span>
        <span className="inline-flex items-center gap-1">
