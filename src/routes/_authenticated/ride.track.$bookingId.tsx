...
function TrackPage() {
  ...
  // Poll live positions for every active trip on this route, in one call — using
  // get_route_active_vehicle_locations instead of the per-booking get_trip_location,
  // since this page is meant to show every active vehicle before the passenger has
  // booked anything (get_trip_location requires an existing booking and would return
  // nothing here even with real GPS data flowing).
  useEffect(() => {
    if (!routeId) {
      setTripLocs({});
      return;
    ...
    const fetchAll = async () => {
      const { data } = await supabase.rpc("get_route_active_vehicle_locations", {
        _route_id: routeId,
      });
      if (cancelled) return;
